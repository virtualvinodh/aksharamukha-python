...
def flush_zip_to_file(file, zip, api):
    if api:
        # in-memory stream
...
def convert_docx(src, tgt, file, nativize=True, pre_options=[], post_options=[], api=False):
    conversion_data = {
        'source': src,
        'target': tgt,
        'nativize': nativize,
    ...
    file = flush_zip_to_file(file, new_zip, api)
