...
def auto_detect(text, plugin=False):
    ...
    if inputScript == 'Bengali':
        ...
        diacritics = ['ā', 'ī', 'ū', 'ṃ', 'ḥ', 'ś', 'ṣ', 'ṇ',
                      'ṛ', 'ṝ', 'ḷ', 'ḹ', 'ḻ', 'ṉ', 'ṟ', 'ṭ', 'ḍ', 'ṅ', 'ñ']
        Itrans = ['R^i', 'R^I', 'L^i', 'L^I',
                  '.N', '~N', '~n', 'Ch', 'sh', 'Sh']
        semitic = ['ʾ', 'ʿ', 'š', 'w']
        BurmeseLOC = ['´', '˝', 'ʻ']
        if 'ʰ' in text:
...
def convert(src, tgt, txt, nativize, preoptions, postoptions):
    ...
    if tgt in GeneralMap.semiticISO.keys():
        if GeneralMap.semiticISO[tgt] != src:
            txt = convert(
                src, GeneralMap.semiticISO[tgt], txt, nativize, preoptions, postoptions)
    ...
    if src in GeneralMap.semiticISO.keys():
        if GeneralMap.semiticISO[src] == tgt:
            ...
            # print('here2')
            txt = convert(
                src, GeneralMap.semiticISO[src], txt, nativize, preoptions, postoptions)
            src = GeneralMap.semiticISO[src]

    if src == "Itrans" and '##' in txt:
        textNew = [[i, word]
                   for i, word in enumerate(txt.split("##")) if (i % 2 == 0)]
        textRest = [(i, word)
                    for i, word in enumerate(txt.split("##")) if (i % 2 == 1)]
    ...
    IndicSemiticMapping = {'Hebrew': 'Hebr', 'Thaana': 'Thaa',
                           'Urdu': 'Arab-Ur', 'Shahmukhi': 'Arab-Pa'}
    ...
    if tgt == 'Hiragana' or tgt == 'Katakana':
        txt = PostProcess.JapanesePostProcess(
            src, tgt, txt, nativize, postoptions)
    ...
    if srcOld == 'Japanese' and tgt != 'Devanagari' and 'siddhammukta' not in postoptions:
        transliteration = Convert.convertScript(
            transliteration, "Devanagari", "ISO")

    if src == tgtOld:
        tgt = tgtOld
        transliteration = Convert.convertScript(
            transliteration, "Devanagari", tgt)
    ...
    if (src not in GeneralMap.SemiticScripts and tgt == 'Arab' and nativize) or 'arabicRemoveAdditionsPhonetic' in postoptions:
        transliteration = getattr(
            PostProcess, 'arabicRemoveAdditionsPhonetic')(transliteration)
    ...
    if 'RemoveDiacritics' in postoptions:
        if tgt == 'Tamil':
            postoptions = map(lambda x: 'RemoveDiacriticsTamil' if x ==
                              'RemoveDiacritics' else x, postoptions)
    ...
    if src == "Tamil" and tgt == "IPA":
        r = requests.get(
            "http://anunaadam.appspot.com/api?text=" + txt + "&method=2")
    ...
    if src == "Itrans" and '##' in txt:
        textConv = {
            **dict(list(json.loads(transliteration.replace("\n", "\\n")))), ** dict(textRest)}
...
def convert_default(src, tgt, txt, nativize=True, post_options=[], pre_options=[]):
    ...
    scriptList = GeneralMap.IndicScripts + \
        GeneralMap.LatinScripts + GeneralMap.SemiticScripts

    preOptionList = list(
        map(lambda x: x[0], getmembers(PreProcess, isfunction)))
    preOptionListLower = list(map(lambda x: x.lower(), preOptionList))

    postOptionList = list(
        map(lambda x: x[0], getmembers(PostProcess, isfunction)))
    postOptionListLower = list(map(lambda x: x.lower(), postOptionList))

    post_options = [option_id for option in post_options for option_id in postOptionList if option.lower(
    ) == option_id.lower()]
    pre_options = [option_id for option in pre_options for option_id in preOptionList if option.lower(
    ) == option_id.lower()]
    ...
    if src not in scriptList:
        script_not_found = 'Source script: ' + src + \
    ...
    if tgt not in scriptList:
        script_not_found = 'Target script: ' + tgt + \
            ' not found in the list of scripts supported. The text will not be transliterated.'
        warnings.warn(script_not_found)
...
def process_default(src, tgt, txt, nativize, post_options, pre_options):
    ...
    if src == 'autodetect':
        ...
        src = [script_id for script_id in scriptList if src.lower() ==
               script_id.lower()][0]

    if tgt.lower() in scriptListLower:
        tgt = [script_id for script_id in scriptList if tgt.lower() ==
               script_id.lower()][0]
...
def process_script_tag(src_tag, tgt_tag, txt, nativize, post_options, pre_options):
    ...
    for scrpt in data_loaded.keys():
        ...
        if 'lang' in data_loaded[scrpt].keys():
            ...
            lang = list(map(lambda x: x.lower(),
                        data_loaded[scrpt]['lang'].split(',')))
        else:
            population = 0
            lang = ''
    ...
    if len(src) > 1:
        warn = "Multiple orthographies, " + \
            ','.join(map(lambda x: x[1], src)) + \
            ", are associated with the input script. The most popular '" + \
            src_pop + "' has been selected"
        warn += "\n Please use the format lang_code-script_code e.g. ur-Arab or pa-Arab to select a particular orthography"

        warnings.warn(warn)
    if len(tgt) > 1:
        warn = "Multiple orthographies: " + \
            ', '.join(map(lambda x: x[1], tgt)) + \
            " are associated with the target script. The most popular '" + \
            tgt_pop + "' has been selected"
        warn += "\n Please use the format lang_code-script_code e.g. ur-Arab or pa-Arab to select a particular orthography"
        warnings.warn(warn)
...
def process_lang_tag(src_tag, tgt_tag, txt, nativize, post_options, pre_options):
    ...
    if src_tag.lower() in ['sa', 'san', 'pi', 'pli']:
        warn = "The input language: " + src_tag + \
            " is script-agnostic and can be written in multiple scripts. The most popular 'Devanagari' has been selected"
    ...
    if tgt_tag.lower() in ['sa', 'san', 'pi', 'pli']:
        warn = "The ouptput language: " + tgt_tag + \
            " is script-agnostic and can be written in multiple scripts. The most popular 'Devanagari' has been selected"
        warn += "\n Please use the format lang_code-script_code e.g. sa-Gran or sa-Sidd to select a particular script"
        warnings.warn(warn)
    ...
    if len(src) > 1:
        warn = "Multiple scripts " + \
            ','.join(map(lambda x: x[1], src)) + \
            " associated with the input language. The most popular '" + \
            src_pop + "' has been selected"
    ...
    if len(tgt) > 1:
        warn = "Multiple scripts " + \
            ','.join(map(lambda x: x[1], tgt)) + \
            " associated with the target language. The most popular '" + \
            tgt_pop + "' has been selected"
        warn += "\n Please use the format lang_code-script_code e.g. pa-Guru or pa-Arab to select a particular script"
        warnings.warn(warn)
...
#     docx_file = ".//aksharamukha//DocxFiles//bhagavata_purana_-_canto_12_-_five_commentaries.docx"
#     import time
#     t1 = time.time()
#     convert_docx_files_tmp('IAST', 'Devanagari', docx_file, nativize = True, post_options = [], pre_options = [])
#     # convert_docx_files_memory(
#     #     'IAST', 'Devanagari', docx_file, nativize=True, post_options=[], pre_options=[])
#     t2 = time.time()
#     print("---bhagavata_purana_-_canto_12_-_five_commentaries.docx file---")
#     print("Time to excute: ", t2-t1," S")
#     import psutil; print("The Memory usage is: ", psutil.Process(os.getpid()).memory_info().rss / 1024 ** 2, " MiB")
